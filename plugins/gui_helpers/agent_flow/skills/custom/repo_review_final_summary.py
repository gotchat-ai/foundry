...
NAME = "custom.repo_review_final_summary"
...
def _verified_files(outputs: List[str], target_folder: str, tool_rows: List[Dict[str, Any]]) -> List[str]:
    ...
    for out in outputs:
        low_out = out.lower()
        if target_tail and target_tail.lower() in low_out:
            inferred = []
            for marker in ("Cargo.toml", "src/main.rs", "src/index.js", "app.py", "README.md"):
                if marker.lower() in low_out:
                    inferred.append(f"{target_tail}/{marker}".replace("//", "/"))
            for path0 in inferred:
