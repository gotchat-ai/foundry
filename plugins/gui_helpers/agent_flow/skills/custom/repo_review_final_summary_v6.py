...
NAME = "custom.repo_review_final_summary_v6"
...
def _target_root_exists(target_folder: str) -> bool:
    return _repo_root_from_target(target_folder) is not None
...
def _implemented_findings(target_folder: str, verified_files: List[str]) -> List[str]:
    texts = _read_verified_sources(target_folder, verified_files)
    findings: List[str] = []
    rust_main = next((v for k, v in texts.items() if k.endswith("src/main.rs")), "")
    ...
    if rust_cargo:
        if "uuid =" in rust_cargo.lower():
            findings.append("- Added the `uuid` dependency so new appointments can use UUID-based identifiers.")
        if "chrono =" in rust_cargo.lower():
            findings.append("- Added the `chrono` dependency for stricter date parsing and time comparisons.")
    if rust_main:
        create_match = re.search(r"async fn create_appointment\s*\([^)]*\)\s*->\s*HttpResponse\s*\{(.*?)\n\}", rust_main, flags=re.DOTALL)
        create_body = str(create_match.group(1) or "") if create_match else ""
        if "Uuid::new_v4()" in rust_main:
            findings.append("- New appointments now use UUID-based identifiers instead of timestamp-derived IDs.")
        if "fs::write(DATA_FILE" in rust_main or "save_appointments(&self)" in rust_main:
            findings.append("- Appointments are now persisted to `appointments.json`, so data can survive restarts.")
        if "validate_times(" in create_body and ("NaiveDateTime::parse_from_str" in rust_main or "end_time must be after start_time" in rust_main):
            findings.append("- Appointment creation now validates datetime format and enforces `end_time` after `start_time`.")
    if node_index:
...
def run(ctx: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    implemented_findings = _implemented_findings(target_folder, verified_files) if (verified_files and (changed_files or implementation_intent)) else []
    if _should_prefer_heuristics(target_folder, verified_files):
    ...
    root = _repo_root_from_target(target_folder)
    if root is not None and not (root / ".git").is_dir():
        git_status = "unavailable for target folder (not a git repo)"
        rag_status = "unavailable for target folder"
