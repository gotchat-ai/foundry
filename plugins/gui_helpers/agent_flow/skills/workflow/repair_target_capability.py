...
NAME = "workflow.repair_target_capability"
PERMISSIONS = ["workflow.repair_target_capability", "workflow.*"]
...
def run(ctx: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    flow_name = str(target.get("flow_name") or params.get("flow_name") or "validated_capability_workflow").strip() or "validated_capability_workflow"

    scaffolded = scaffold_capability_run(
        ctx,
        {
    ...
    fix_summary = "Rebuilt the failing workflow target using the capability-template scaffold and regenerated the affected skill files so the sandbox can retest a substantive generated workflow."
    if any("timed out" in bug.lower() for bug in bugs):
        fix_summary = "Rebuilt the failing workflow target into a bounded capability-template variant and regenerated skill files to reduce sandbox timeouts while preserving required capabilities."
...
TOOL_SPEC = {
    "id": NAME,
    "category": "workflow",
    "label": "Workflow Repair Target Capability",
    "description": "Deterministically rebuild a failing workflow target from generic capability templates so the sandbox can retest it.",
