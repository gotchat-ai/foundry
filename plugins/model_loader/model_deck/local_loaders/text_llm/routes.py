...
LOADER_ID = "model_loader.model_deck.text_llm"
...
def load(request: Request, settings: Dict[str, Any]) -> Dict[str, Any]:
    gguf_settings = map_gguf_settings(settings, request=request)
...
def install(app) -> None:
    ...
    if isinstance(reg, dict):
        reg.setdefault(LOADER_ID, type("DeckTextLLM", (), {"id": LOADER_ID, "name": "Model Deck Text LLM (GGUF)", "load": staticmethod(load), "unload": staticmethod(unload)})())

    r = APIRouter()

    @r.get("/v1/model_loader/model_deck/text_llm/status")
