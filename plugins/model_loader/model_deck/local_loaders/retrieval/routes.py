...
LOADER_ID = "model_loader.model_deck.retrieval"
...
def load(request: Request, settings: Dict[str, Any]) -> Dict[str, Any]:
    gguf_settings = map_gguf_settings(settings)
...
def install(app) -> None:
    ...
    if isinstance(reg, dict):
        reg.setdefault(LOADER_ID, type("DeckRetrieval", (), {"id": LOADER_ID, "name": "Model Deck Retrieval (GGUF)", "load": staticmethod(load), "unload": staticmethod(unload)})())

    r = APIRouter()

    @r.get("/v1/model_loader/model_deck/retrieval/status")
